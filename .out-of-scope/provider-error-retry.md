...
**Decision:** archLoop does not retry on provider errors (rate limits, auth failures, quota errors, network timeouts, etc.).

**Why:**

- archLoop shells out to provider CLIs (e.g. Claude Code) — it doesn't own the API connection or error interface.
...
**Principle:** Error handling and retry logic belong in the provider/harness layer, not in archLoop. archLoop fails fast on provider errors.
