...
Init copies the selected role prompt templates into `.sandcastle/agents/`, copies their bundled skills into `.sandcastle/skills/`, and writes a role manifest such as `.sandcastle/agent-profiles.json`. Users can then customize `main.mts` to compose the template's default workflow with any additional preset agent roles.
...
1. As a Sandcastle user, I want `init` to ask whether I want to add preset agent roles, so that I can start with more than the template's default prompt.
2. As a Sandcastle user, I want to skip preset agent roles, so that the current init flow remains simple.
3. As a Sandcastle user, I want to select multiple preset agent roles during init, so that I can compose a project-specific agent team.
4. As a Sandcastle user, I want to add a Mini Program developer preset agent, so that WeChat Mini Program projects get relevant prompt guidance immediately.
5. As a Sandcastle user, I want to add a reviewer preset agent, so that review behavior is reusable across templates.
6. As a Sandcastle user, I want to add a planner preset agent, so that planning behavior can be reused outside the default planner templates.
7. As a Sandcastle user, I want to add a merger preset agent, so that merge and conflict handling can be expressed as a role.
8. As a Sandcastle user, I want each preset agent role to come with bundled skills, so that I do not need to manually discover or copy supporting skill documents.
9. As a Sandcastle user, I want preset agent files to live under `.sandcastle/agents/`, so that I can inspect and edit role prompts directly.
10. As a Sandcastle user, I want bundled skill files to live under `.sandcastle/skills/`, so that the role's dependencies are visible and project-scoped.
11. As a Sandcastle user, I want a manifest that records selected preset agent roles, so that `main.mts` and future tooling can discover them consistently.
12. As a Sandcastle user, I want role prompts to reference their bundled skills using project-relative paths, so that agents can read the skills from the sandbox worktree.
13. As a Sandcastle user, I want preset agent roles to work with Docker and Podman sandbox providers, so that the feature fits the existing sandbox provider model.
14. As a Sandcastle user, I want preset agent roles to work without depending on a specific agent provider's native skill system, so that Codex, Cursor, Claude Code, and other agents can all benefit.
15. As a Sandcastle user, I want provider and model recommendations for each preset agent role, so that I can quickly choose sensible defaults.
16. As a Sandcastle user, I want to override the recommended provider and model in `main.mts`, so that preset roles do not lock me into one agent.
17. As a Sandcastle user, I want the template's existing prompt templates to remain unchanged unless I select additional roles, so that init remains backward compatible.
18. As a Sandcastle user, I want generated files to be ordinary Markdown and JSON, so that I can edit them without learning a new configuration language.
19. As a Sandcastle user, I want selected roles to be copied into the config directory, so that the project remains understandable when shared with teammates.
20. As a Sandcastle user, I want private host skills to remain out of scope for the first version, so that init does not accidentally expose personal auth, cache, or home-directory data.
21. As a Sandcastle user, I want bundled preset skills to be safe to commit by default, so that project teammates can reproduce the same role behavior.
22. As a Sandcastle user, I want the init summary and next steps to explain how to use selected roles from `main.mts`, so that I know what to edit next.
23. As a Sandcastle user, I want preset agent roles to be optional per project, so that different projects can have different role sets.
24. As a Sandcastle user, I want role prompts to include concrete responsibilities, constraints, and completion expectations, so that the agent role is more than a name.
25. As a Sandcastle user, I want bundled skills to be de-duplicated when multiple roles need the same skill, so that `.sandcastle/skills/` stays tidy.
26. As a Sandcastle user, I want init to show short descriptions for preset agents, so that I can choose roles without opening source files.
27. As a Sandcastle user, I want invalid or missing preset metadata to fail tests before release, so that broken presets do not ship.
...
The Mini Program developer preset is a good initial role because it demonstrates the value of bundled domain skills. Reviewer, planner, and merger presets are also natural candidates because they map directly to existing Sandcastle template concepts.

Future versions can add private skill import, host skill mounting, `sandcastle agents add`, `sandcastle agents list`, and runtime helpers after the preset role file format proves stable.
