...
This preserves Sandcastle's current strengths:
...
1. As a Sandcastle user, I want `init` to distinguish between the default agent and installed runtimes, so that choosing one scaffold example does not restrict the whole project.
2. As a Sandcastle user, I want to install multiple agent runtimes in the same sandbox, so that one project can use Codex, Cursor, Claude Code, and others together.
3. As a Sandcastle user, I want to select Cursor as the default implementer while also installing Codex for planning and review, so that the scaffold matches my intended workflow.
4. As a Sandcastle user, I want to install only the runtimes I plan to use, so that the sandbox image stays smaller and easier to understand.
5. As a Sandcastle user, I want the generated Dockerfile or Containerfile to install every selected runtime, so that the sandbox can execute every referenced agent provider.
6. As a Sandcastle user, I want `.env.example` to include the relevant auth hints for all installed runtimes, so that setup is complete on first run.
7. As a Sandcastle user, I want auth/config mounts to be generated for all selected runtimes, so that the container can authenticate each provider consistently.
8. As a Sandcastle user, I want `main.mts` to still show one simple default path, so that the scaffold remains approachable even when multi-agent support is enabled.
9. As a Sandcastle user, I want to hand-edit `main.mts` to orchestrate multiple providers after init, so that I can express planner/implementer/reviewer/merger workflows freely.
10. As a Sandcastle user, I want the init UI to make it obvious that selecting a default agent does not disable other runtimes, so that the mental model is clear.
11. As a Sandcastle user, I want non-interactive init flags to support multiple runtimes, so that CI, scripts, and reproducible project bootstrap flows work.
12. As a Sandcastle user, I want preset agent roles to remain compatible with multi-runtime projects, so that a role can recommend a provider without forcing sandbox changes by hand.
13. As a Sandcastle user, I want the generated scaffold to support role-level provider mixing, so that a reviewer role can use one provider while an implementer role uses another.
14. As a Sandcastle user, I want projects to remain valid when only one runtime is selected, so that the new model stays backward compatible with simpler setups.
15. As a Sandcastle user, I want sandbox provider behavior to stay predictable across Docker and Podman, so that multi-runtime support does not depend on one provider only.
16. As a Sandcastle user, I want the README and init next steps to explain the difference between default agent and installed runtimes, so that I know how to customize the scaffold.
17. As a Sandcastle user, I want runtime selection to affect only sandbox/image/auth setup, so that changing my orchestration later does not require a redesign of init concepts.
18. As a Sandcastle user, I want one project to support multiple LLM models across providers, so that role-specific model choice is natural.
19. As a Sandcastle user, I want init to fail early if I request an unknown runtime, so that scripting mistakes are caught immediately.
20. As a Sandcastle user, I want generated files to reflect only the runtimes I selected, so that the config directory stays legible.
...
- Supporting arbitrary external CLIs that are not Sandcastle agent providers.

## Further Notes

This feature should be framed as a clarification of Sandcastle's architecture, not just a convenience tweak. Sandcastle already supports different **agent providers** at runtime; the problem is that `init` currently communicates and scaffolds the project as if one selected agent defines the whole project.
