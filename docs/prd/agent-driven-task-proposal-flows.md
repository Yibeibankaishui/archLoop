...
Sandcastle Hub task board commands such as PRD decomposition and task triage currently risk behaving like deterministic CLI helpers even though their primary value depends on semantic judgment. A regex-based `from-prd` command can prove that Beads writes work, but it cannot provide the same experience as an agent-guided `$to-issues` session: discussing slice boundaries, checking dependencies, explaining AFK/HITL classification, and iterating until the user approves the breakdown. A rule-based `triage` command has the same problem: triage is not just label assignment, it is a conversation that gathers context, recommends state transitions, asks for missing information, and produces durable notes.

Users need `sandcastle tasks from-prd` and `sandcastle tasks triage` to feel like first-class Sandcastle orchestration, not hidden deterministic scripts. These commands should use agents for judgment while preserving Hub's ownership of validation, confirmation, task store writes, run artifacts, and synchronization boundaries.

## Solution

Introduce agent-driven task commands backed by proposal flows. A proposal flow is a no-sandbox Hub flow that gathers required context, invokes an agent through Sandcastle's agent provider abstraction, lets the user refine a proposal through a CLI-managed proposal session, and accepts only a schema-validated task proposal as the final write request. Hub then validates the proposal, detects forbidden mutations, asks for confirmation or applies guarded `--yes` behavior, and writes the approved changes to the local task store.

`sandcastle tasks from-prd <prd-ref>` becomes the primary shortcut for the `prd-decomposition` proposal flow. It reads the PRD and related context, lets the user discuss vertical slices with the agent, and creates Beads tasks and dependency edges only after approval. `sandcastle tasks triage` becomes the primary shortcut for the `triage` proposal flow. It reads inbox and needs-info tasks, lets the user discuss the recommendations with the agent, and applies approved status, label, dependency, and comment updates locally.

`sandcastle run <project> --flow <id> --input <value>` should also expose these flows through the same flow execution path. The shortcut commands remain the recommended user interface, while `run --flow` provides a uniform advanced entry point for CLI automation and future GUI integration.

Hub-wide agent configuration selects provider, model, and provider-specific options by reusable stage role. The first version exposes role-level configuration through CLI commands and uses these roles for flow stages such as planning, triage, implementation, review, merge, and recovery. Proposal flows use the planning or triage roles and always run no-sandbox, but they are still Sandcastle flows: they use Sandcastle prompts, agent providers, structured output, run directories, and Hub task board writes.

## User Stories

1. As a Sandcastle Hub user, I want PRD decomposition to be performed by an agent, so that the task breakdown reflects semantic understanding rather than Markdown shape.
2. As a Sandcastle Hub user, I want `sandcastle tasks from-prd` to open an interactive proposal session, so that I can refine slices before tasks are created.
3. As a Sandcastle Hub user, I want the PRD decomposition agent to propose vertical slices, so that each task is independently grabbable and verifiable.
4. As a Sandcastle Hub user, I want each PRD-derived slice to include AFK/HITL classification, so that agent-owned and human-owned work are explicit.
5. As a Sandcastle Hub user, I want each PRD-derived slice to include acceptance criteria, so that implementers and reviewers know what done means.
6. As a Sandcastle Hub user, I want each PRD-derived slice to include dependency suggestions, so that Beads can represent the true ready queue.
7. As a Sandcastle Hub user, I want the agent to explain its slice and dependency rationale, so that I can judge whether the breakdown is correct.
8. As a Sandcastle Hub user, I want to ask the decomposition agent to split, merge, reorder, or reclassify slices, so that the final task graph matches my intent.
9. As a Sandcastle Hub user, I want to approve the final PRD decomposition before it writes Beads tasks, so that agent judgment never silently mutates the local task store.
10. As a Sandcastle Hub user, I want PRD-derived tasks to default to inbox, so that new work still passes through the task board unless I explicitly choose ready states.
11. As a Sandcastle Hub user, I want an interactive PRD session to optionally create AFK slices as ready_for_agent and HITL slices as ready_for_human, so that confirmed breakdowns can skip redundant triage.
12. As a Sandcastle Hub user, I want `from-prd --yes` to run a one-shot proposal and create inbox tasks by default, so that scripted workflows remain possible.
13. As a Sandcastle Hub user, I want high-severity PRD warnings to block unattended ready-state creation, so that unclear or risky slices are not sent directly to agents.
14. As a Sandcastle Hub user, I want `sandcastle tasks triage` to be performed by an agent, so that triage recommendations reflect task body, comments, dependencies, and codebase context.
...
22. As a collaborator, I want remote GitHub Issues to be updated by `sandcastle tasks sync`, not by proposal flows, so that sync conflicts and remote failures have one consistent owner.
23. As a Sandcastle Hub user, I want proposal sessions to save transcripts and final proposals in the Hub run directory, so that I can inspect how a task graph or triage decision was produced.
24. As a Sandcastle Hub user, I want Beads tasks to store concise run references instead of full transcripts, so that task history remains readable.
25. As a Sandcastle Hub user, I want proposal flows to use structured output, so that Hub can validate final proposals before writing them.
26. As a Sandcastle Hub user, I want invalid or missing structured output to fail without writing Beads, so that malformed agent responses are safe.
27. As a Sandcastle Hub user, I want proposal flows to run no-sandbox, so that they can naturally read host PRDs, Beads data, and Hub context without mount complexity.
28. As a Sandcastle Hub user, I want proposal flows to be allowed to explore extra context through normal agent behavior, so that high-capability agents can inspect relevant docs or code when useful.
29. As a Sandcastle Hub user, I want proposal flows to be checked after running for repo or Beads mutations, so that accidental writes are detected even without shell allowlists.
30. As a Sandcastle Hub user, I want detected proposal-flow mutations to fail the flow without automatic rollback, so that Sandcastle does not destroy concurrent user work.
31. As a Sandcastle Hub user, I want a Hub-wide agent config, so that all projects use a consistent provider/model policy for planning, triage, implementation, review, merge, and recovery.
32. As a Sandcastle Hub user, I want CLI commands to show and set Hub agent roles, so that I can configure the same settings a future GUI will edit.
33. As a Sandcastle Hub user, I want missing agent role config to trigger interactive setup in TTY mode, so that first-run setup is smooth.
34. As a Sandcastle Hub user, I want missing agent role config to fail clearly in non-interactive or `--yes` mode, so that scripts do not hang.
35. As a Sandcastle Hub user, I want agent role config to avoid storing credentials, so that provider/model choices stay separate from Hub env files and login state.
36. As a Sandcastle Hub user, I want run readiness checks before invoking a provider, so that missing credentials or login state are reported before a proposal session starts.
37. As an advanced user, I want `sandcastle run . --flow prd-decomposition --input <prd-ref>` to work, so that proposal flows can be scripted through the general flow interface.
...
40. As a Sandcastle maintainer, I want deterministic PRD decomposition and triage heuristics removed from the user main path, so that judgment-producing task commands behave consistently.
41. As a Sandcastle maintainer, I want deterministic helpers retained only for tests or explicit internal fallback, so that tests can run without requiring a live agent.
42. As a Sandcastle maintainer, I want proposal prompts to be Sandcastle-owned, so that default behavior is versioned with the package and does not depend on user-local skills.
43. As a Sandcastle maintainer, I want proposal prompts to incorporate the methodology of the to-issues and triage skills, so that the productized flow keeps the familiar experience.
44. As a Sandcastle maintainer, I want ADRs for proposal sessions, no-sandbox mutation detection, and Hub-wide agent roles, so that future contributors understand the trade-offs.
45. As a Sandcastle maintainer, I want README, bundled Sandcastle skill, QA docs, and roadmap to describe the new behavior, so that users do not expect regex-based PRD parsing or direct remote issue writes.
...
- Treat `sandcastle tasks from-prd` and `sandcastle tasks triage` as agent-driven task commands. They remain user-friendly shortcuts but internally execute proposal flows.
- Expose `prd-decomposition` and `triage` through `sandcastle run <project> --flow <id> --input <value>` as advanced entry points.
...
- Add Hub-owned prompts for `prd-decomposition` and `triage`. These prompts should be packaged with Sandcastle and should not depend on host-local skill paths.
...
- Require final proposals to be emitted as Sandcastle structured output. Hub validates the final task proposal schema before any Beads write.
...
- Add Hub-wide agent config stored in the Sandcastle user data directory, not in target project repos.
...
- Update public documentation, the bundled Sandcastle usage skill, QA docs, and roadmap to describe agent-driven proposal flows and the new agent config model.
...
- Add tests for `sandcastle run --flow --input`: dispatch to proposal flows, input validation, missing config, and shared execution path with task shortcut commands.
...
- Removing `sandcastle init` or old scaffolded `main.ts` / `main.mts` workflows.
...
The proposal session model is intentionally CLI-first but GUI-ready. The same transcript, final proposal, run artifacts, flow input schema, and Hub agent config can later power a graphical Sandcastle Hub without changing the task-board semantics.
