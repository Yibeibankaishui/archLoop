...
| Proposal Session | Approve                                                                                                | CLI-only, disabled with reason when invalid             | Approve is a precondition, not a direct remote mutation.                 |
| Proposal Session | Reject                                                                                                 | CLI-only                                                | Dismisses the proposal without local writes.                             |
| Proposal Session | Revise                                                                                                 | CLI-only                                                | Re-enters the proposal session loop.                                     |
| Proposal Session | Apply locally                                                                                          | CLI-only, disabled with reason until approved           | Writes local Beads tasks only; GitHub stays separate.                    |
...
- Primary visible CTAs must never become unexplained CLI-only fallbacks; if v0 cannot provide a desktop action path, the control must be disabled with a reason that names the CLI recovery path.
