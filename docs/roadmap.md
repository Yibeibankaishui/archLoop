...
- [ ] 实现 fully automatic Hub landing：以 Hub-managed local ref 为默认权威目标，按任务执行 durable candidate/verification/fenced-CAS/close transaction，以 ordered speculative chain 并行验证并按 FIFO 串行 landing；checkout、best-effort publication 和 cleanup 作为独立 durable projection 自动重试（见 [fully-automatic-hub-landing](./prd/fully-automatic-hub-landing.md)、[#247](https://github.com/Yibeibankaishui/archLoop/issues/247)、[ADR-0035](./adr/0035-hub-landing-uses-durable-fenced-transactions.md)）。
- [ ] 将 Hub Beads task store 外置到 stable Hub project directory，注入显式 `BEADS_DIR`、安装 Git-ignored `.beads/redirect`、自动迁移 legacy store、为执行 Agent 生成 immutable task snapshot，并为已有 task branch 的 runtime/export 变更提供安全兼容（见 [#247](https://github.com/Yibeibankaishui/archLoop/issues/247)、[ADR-0036](./adr/0036-hub-owns-the-beads-task-store.md)）。
