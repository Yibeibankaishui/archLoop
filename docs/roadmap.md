...
- [x] 强化 Hub flow lifecycle 与 batch merge preflight：rerun 可复用已有未合并分支 work，transition 后校验 task projection，merge summary 解释 selected/skipped/blocked 原因，将 `.beads/` runtime/export 文件排除在普通代码分支 merge 之外，在无 ready 任务时恢复同 flow 的旧 unfinished batch merge，并在 Git merge conflict 时调用配置好的 `merge` agent role 解决冲突后再继续 verification/close（见 [#120](https://github.com/Yibeibankaishui/sandcastle/issues/120)、[#121](https://github.com/Yibeibankaishui/sandcastle/issues/121)、[#122](https://github.com/Yibeibankaishui/sandcastle/issues/122)、[#123](https://github.com/Yibeibankaishui/sandcastle/issues/123)、[#124](https://github.com/Yibeibankaishui/sandcastle/issues/124)、[#125](https://github.com/Yibeibankaishui/sandcastle/issues/125)、[#126](https://github.com/Yibeibankaishui/sandcastle/issues/126)、[ADR-0027](./adr/0027-hub-beads-runtime-files-stay-local.md)）。
...
- [x] 实现 Hub provider auth namespace：`sandcastle auth show/path/login` 负责 Codex/GitHub Hub-owned session 目录，`env init` 保持 key/value 边界并提示 Codex API billing vs CLI login session（见 [#108](https://github.com/Yibeibankaishui/sandcastle/issues/108)、[ADR-0028](./adr/0028-hub-auth-namespace-owns-provider-sessions.md)）。
