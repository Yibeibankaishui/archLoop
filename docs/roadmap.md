...
- [x] 实现 `archloop tasks recover <task-selector>`：释放 stale claim、恢复 recoverable `failed` 任务，并处理已 merge 分支上的 `close_failed`（见 [#73](https://github.com/yibeibankaishui/archloop/issues/73)）。
...
- [x] 实现 `archloop project configure` 与 Hub project development contract 持久化输出（见 [#173](https://github.com/yibeibankaishui/archloop/issues/173)）。
