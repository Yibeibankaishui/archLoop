...
- [x] 隔离 CLI 测试与真实 Hub project registry：测试固定使用 test-owned `XDG_DATA_HOME` 与独立 config/cache roots，registry I/O 在测试中拒绝真实用户数据目录，suite sentinel 校验真实 registry 未被改写，并提供 `archloop project prune-test-fixtures` 以 dry-run 诊断、backup 后清理 historically leaked `cli-host-*` / `cli-resolve-*` 夹具与对应 path-hash run 目录（见 [#275](https://github.com/Yibeibankaishui/archLoop/issues/275)、parent [#263](https://github.com/Yibeibankaishui/archLoop/issues/263)）。
