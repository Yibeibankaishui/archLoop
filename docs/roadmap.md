...
- [ ] 实现 fully automatic Hub landing：以 Hub-managed local ref 为默认权威目标，按任务执行 durable candidate/verification/fenced-CAS/close transaction，以 ordered speculative chain 并行验证并按 FIFO 串行 landing；checkout、best-effort publication 和 cleanup 作为独立 durable projection 自动重试（见 [fully-automatic-hub-landing](./prd/fully-automatic-hub-landing.md)、parent [#247](https://github.com/Yibeibankaishui/archLoop/issues/247)、[#249](https://github.com/Yibeibankaishui/archLoop/issues/249)、[#252](https://github.com/Yibeibankaishui/archLoop/issues/252)、[#253](https://github.com/Yibeibankaishui/archLoop/issues/253)、[#255](https://github.com/Yibeibankaishui/archLoop/issues/255)、[#256](https://github.com/Yibeibankaishui/archLoop/issues/256)、[#257](https://github.com/Yibeibankaishui/archLoop/issues/257)、[#258](https://github.com/Yibeibankaishui/archLoop/issues/258)、[#260](https://github.com/Yibeibankaishui/archLoop/issues/260)、[#261](https://github.com/Yibeibankaishui/archLoop/issues/261)、[ADR-0035](./adr/0035-hub-landing-uses-durable-fenced-transactions.md)）。
      Current slice: After local shipped proof, Hub enqueues a durable best-effort publication outbox ([#258](https://github.com/Yibeibankaishui/archLoop/issues/258)) keyed by transaction/remote/ref/candidate/expected OID. Publication observes the remote and uses exact force-with-lease semantics; network, credential, protected-branch, and unknown outcomes stay `target_publish_pending` without undoing shipped or mixing with GitHub task sync. Discovering `origin` alone never enables publication. After landing, Hub also enqueues a durable checkout projection ([#257](https://github.com/Yibeibankaishui/archLoop/issues/257)). Un-checked-out host branches advance with OID CAS; a checked-out branch fast-forwards only in its owning worktree when Git proves the index, WIP, operation state, untracked paths, sparse checkout, submodules, and other worktrees safe. Unsafe state stays `checkout_sync_pending` without touching user files or `shipped`, and retries on a later mutating entry point. Upgrade adopts pre-transaction landing history without false completion ([#255](https://github.com/Yibeibankaishui/archLoop/issues/255)). Concurrent landing coordinators share a nonce/process-start/boot landing lease ([#256](https://github.com/Yibeibankaishui/archLoop/issues/256)). Independent merge-ready tasks land in separate transactions with bounded Agent repair ([#253](https://github.com/Yibeibankaishui/archLoop/issues/253)). Interrupted local landing transactions resume from durable evidence ([#252](https://github.com/Yibeibankaishui/archLoop/issues/252)). `tasks doctor` stays read-only. Ordered speculative chains, FIFO quiet-wait, host contribution ordering, and required publication remain.
