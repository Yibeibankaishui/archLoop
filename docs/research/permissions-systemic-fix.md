...
- No `--build-arg AGENT_UID=$(id -u)` path in `archloop docker build-image` despite ADR-0005 explicitly listing it as the planned escape hatch.
...
- `archloop docker build-image` (and `DockerLifecycle.buildImage`) gain a `buildArgs` option and pass `--build-arg AGENT_UID=$(process.getuid())` / `--build-arg AGENT_GID=$(process.getgid())` by default on Linux/macOS.
...
Before `docker run`, run `docker image inspect <img> --format '{{.Config.User}}'` and compare to host UID. If mismatched and the image was not built with `--build-arg AGENT_UID`, throw a clear error pointing to `archloop docker build-image`. Mirror the Podman pre-flight already in place.
