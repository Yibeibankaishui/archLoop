...
整理对象：`beads` / `bd`
研究时间：2026-06-14；2026-06-20 追加 archLoop Hub 集成验证
结论版本：基于本机安装的 `bd 1.0.4` CLI 帮助、Context7 上的 `/gastownhall/beads` 文档，以及 archLoop Hub QA 中的真实命令行为。
...
2026-06-20 再次核对 `bd 1.0.4` 和官方 CLI reference 后，以下语义对 archLoop 集成尤其重要：
...
- Beads 的 stored status 仍是 `open`、`in_progress`、`blocked`、`deferred`、`closed` 这一级；archLoop 的 `ready_for_agent`、`reviewing`、`waiting_for_merge` 等更细状态需要通过 labels/metadata 在 Beads 上投影。
...
archLoop QA 中观察到：即使执行 `bd --readonly show ...`，Beads 也可能因为 auto-export 尝试执行 `git add`，在 Codex sandbox 里表现为 `.git/index.lock` 权限错误噪音。对上层 CLI 来说，这意味着：

- 只读读取最好统一加 `--readonly`，并评估是否需要 `--sandbox`。
- archLoop 不应让 `tasks list/show` 这类只读命令因为 auto-export 的 git 副作用而吓到用户。
- 如果继续把 Beads 作为内部模块，archLoop 应该在错误包装里区分“真实读取失败”和“auto-export/git add 噪音”。
...
## 2026-06-20 archLoop Hub QA 结论

这轮 QA 暴露的问题不是 Beads 本身的任务模型不适合，而是 archLoop 把更细的 Hub 状态投影到 Beads 时，写入策略不够严格。

### 事故现象

`archloop run . --flow with-review` 中，一批 task 实现和 review 完成后，merge 阶段只选中了部分任务。排查发现：
...
另一个任务 `archloop-3n0` 仍保持 failed，原因不同：它的 implement agent 因 Cursor provider `socket hang up` 失败，commit count 为 0，分支没有任何未合并 work。这类任务应通过 `archloop tasks recover <id>` 回到 `ready_for_agent` 后重新运行，而不能直接推进到 merge。

### 对 archLoop 集成的设计约束

1. **Hub 状态写入必须有唯一入口。** 所有状态变化都应通过类似 `transitionHubTaskStatus()` 的函数，统一 Beads lifecycle status、Hub status label、metadata。
2. **状态 labels 要 canonical replace。** archLoop 只应保留一个自己管理的状态 label。实现时先读当前 labels，保留用户自定义 labels，去掉所有 archLoop 状态 labels，再用 `bd update --set-labels` 写回。`blocked`、`failed`、`done`、`wontfix` 等通用词如果作为 archLoop 状态 label 使用，就必须在文档中声明为保留 label；否则投影层应以 `metadata.hubStatus` 为准，labels 只作为显示/兼容 fallback。
...
12. **对外隐藏 Beads，内部尊重 Beads。** 用户命令应继续是 `archloop tasks ...`；archLoop 内部可以使用 `bd`，但要把 Beads 的副作用、错误信息、版本差异包装成 archLoop 语义。

### 建议落地顺序

1. 先做真实 `bd 1.0.4` characterization tests：`--set-labels`、JSON/数组 metadata、`--unset-metadata`、只读参数、auto-export 副作用。
2. 提取纯函数：archLoop 管理 label 集合、canonical label 计算、metadata patch/clear 规则、transition 后验证规则。
...
7. 最后增加 `archloop tasks doctor` / `archloop tasks repair-state`，再更新 README、skill、changeset。
...
- archLoop Hub QA 验证：`bd update --set-labels`、`bd show --long --thread --refs`、`bd comments`、`archloop run . --flow with-review` resumed batch merge
