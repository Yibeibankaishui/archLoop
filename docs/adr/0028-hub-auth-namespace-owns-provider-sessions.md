...
**archLoop Hub** separates API-key style credentials from provider login
sessions. `archloop env` remains the Hub env file interface for keys and
tokens. `archloop auth` owns provider CLI login/session setup, inspection,
and provider auth paths.

## Decision

Add a dedicated `archloop auth` command namespace for Hub provider auth:
...
`archloop env init` may explain that a provider has a login/session path and
point users to `archloop auth login <provider>`, but it must not manage
...
Hub-owned auth state defaults to the **archLoop user data directory**, so it
...
- Auth session files stay out of target repos and out of `.archloop/`
  project scaffolds.
...
1. **Put provider login inside `archloop env init`** -- rejected because env
...
4. **Add a dedicated `archloop auth` namespace** -- chosen because it keeps a
