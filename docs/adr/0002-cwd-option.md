...
`promptFile` is **not** re-rooted under `cwd`. It is resolved as an ordinary Node file path, against `process.cwd()`. Only host-repo-derived paths (`.archloop/worktrees/`, `.archloop/.env`, `.archloop/logs/`, `.archloop/patches/`) follow `cwd`.

The rejected alternative was to resolve relative `promptFile` against `cwd`, so `"./.archloop/prompt.md"` would always mean "the prompt inside the archloop project." That reads natural in examples, but it makes `promptFile` behave unlike every other file path a user passes to a Node API — surprising when debugging.
