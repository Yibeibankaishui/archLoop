...
repo itself is a worktree** and when archLoop **creates a worktree** via the
...
1. **Mount the parent `.git` dir at a deterministic POSIX path** —
   `/.archloop-parent-git`. This gives the parent git directory a stable,
...
2. **Create a corrected `.git` file** with the `gitdir:` path rewritten to
   ...
   `gitdir: /.archloop-parent-git/worktrees/abc`. Mount this file at
...
5. Remaps the parent `.git` dir mount to `/.archloop-parent-git`.
...
- **archLoop-created worktree** — adds a new overlay mount, since the `.git`
...
- **Entrypoint script**: Have the container's init script detect and rewrite
  Windows paths. Rejected because it pushes archLoop-specific logic into the
...
- A small temp file is created per sandbox session. It is cleaned up when the
  ...
  normal temp cleanup.
- The `/.archloop-parent-git` path is reserved inside the sandbox. This is
