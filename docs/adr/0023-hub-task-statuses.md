...
**Sandcastle Hub** task board v1 uses these canonical task statuses: `inbox`, `needs_info`, `ready_for_agent`, `ready_for_human`, `blocked`, `implementing`, `reviewing`, `waiting_for_merge`, `merging`, `done`, `wontfix`, `failed`, and `sync_conflict`. Ambiguous or transient terms such as `pending`, `triaging`, `waiting_for_review`, `planning`, `reserved`, `claimed`, and `deferred` are not task statuses.
