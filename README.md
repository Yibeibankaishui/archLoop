...
For deterministic CI or redirected logs, task-board flows accept explicit plain output: `archloop run --flow no-review --output plain` and `archloop run --flow with-review --output plain`. Plain mode writes one append-only physical line per canonical lifecycle event with stable field ordering, escaped values, no ANSI cursor rewriting, and user-facing task stages. It identifies the Hub project, flow, selected tasks, run id, Hub run directory, completed batch/task counts, and final outcome; an initially empty queue ends successfully with `Nothing to run`. Agent prose, tool arguments, and direct agent-startup decoration stay out of plain output and remain available in the Hub run directory. Omitting `--output` keeps the existing terminal behavior. Plain mode currently applies only to the two task-board flows.
