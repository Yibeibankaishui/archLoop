...
Runs the `triage` proposal flow for `inbox` and `needs_info` Hub tasks. It invokes the Hub `triage` agent through a proposal session, lets you refine recommendations, and applies approved status, label, dependency, and comment updates locally. Approved comments start with `> *This was generated by AI during triage.*`. Pass `--yes` to run a one-shot proposal and apply only high-confidence, low-risk, non-closing decisions.
...
The first available task-board flows are `no-review` and `with-review`. Proposal flows `prd-decomposition` and `triage` also appear in the flow registry with typed input schemas. `sandcastle tasks from-prd` and `sandcastle run --flow prd-decomposition --input <prd-ref>` share the same agent-driven PRD decomposition path. `sandcastle tasks triage` and `sandcastle run --flow triage` share the same agent-driven triage path. Pass `--yes` on either entry point for one-shot proposal execution.
...
| `--yes`   | No       | Run proposal flows in one-shot mode without interactive prompts                                                         |
