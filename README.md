...
Task-board runs default to `--output auto`. A capable interactive TTY gets a bounded live view with a compact run header, stable current-batch task rows, canonical stages, elapsed time, durable logs, completed-batch scrollback, and a final outcome with all five task counts. The renderer never enters the alternate screen and does not show raw agent prose, tool arguments, percentages, or ETA claims. It switches between wide and stacked layouts as the terminal resizes, restores the cursor on success, failure, cancellation, and abnormal cleanup, and falls back to plain output for redirected stdout, CI, `TERM=dumb`, unsupported cursor control, or unsafe terminal width or height. In the live view, `NO_COLOR` or `--no-color` removes color without removing textual labels or status symbols.

For deterministic CI or redirected logs, task-board flows use plain output automatically; it can also be selected explicitly with `archloop run --flow no-review --output plain` or `archloop run --flow with-review --output plain`. Plain mode writes append-only physical lines with stable field ordering, escaped values, no ANSI cursor rewriting, and user-facing task stages. It identifies the Hub project, flow, selected tasks, run id, Hub run directory, all five outcome counts, and the final outcome. Failed task attention lines retain the failed stage, concise diagnostic, relevant log path, and applicable recovery command. Agent or sandbox implementation/review failures use `archloop tasks recover <selector>`; inconsistent claim or projection state uses `archloop tasks repair-state <selector>`; dirty overlap lists exact paths and tells you to commit, stash, or discard them. Work left in `waiting_for_merge` tells you to rerun the same `archloop run --flow <id>` command so automatic batch resume applies; there is no `archloop run --resume` contract. Agent prose, tool arguments, and direct agent-startup decoration stay out of plain output and remain available in the Hub run directory. Plain mode currently applies only to the two task-board flows.

For automation, use `archloop run --flow <no-review|with-review> --output json`. JSON mode writes JSONL to stdout: every physical line is one JSON object, normal lifecycle failures remain JSON, and prompts, human status text, ANSI control sequences, agent startup text, and raw agent output are excluded. Schema version `1` records have `schemaVersion`, output `eventId` / `sequence`, `timestamp`, `type`, `runId`, and `flowId`; canonical lifecycle records also include `sourceEventId` / `sourceSequence`, optional `batchId` / `taskId`, and event-specific fields under `data`. `task_attention` records expose the failed stage, diagnostic, log path, recovery command, and blocking paths when applicable. The final `run_completed` record exposes `outcome`, all five task counts, completed batch/task totals, stop reason, log directory, and `exitCode`; cancellation uses outcome `cancelled` and exit `130`. JSON string escaping keeps whitespace, newlines, quotes, paths, and diagnostics inside one record. Emergency process diagnostics that cannot be represented as lifecycle events may still use stderr. The schema is additive within version `1`; consumers should ignore unknown fields.
