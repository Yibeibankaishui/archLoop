...
Shows the Hub task board view for Beads data in the selected Hub project: a header (`archLoop · <project>`), status badges (`todo` / `in_progress` / `done`), per-bucket task groups with full Beads ids (no 1-based ordinals), and a tip footer. Each row may show a trailing remote badge: dim cyan `github#N` when synced, dim `local-only` when push is pending, or yellow `sync-conflict` when local and remote disagree. Rows with no remote link omit the badge. Tasks the interrupted-execution detector flags as stuck in `implementing` / `reviewing` / `merging` with no live worktree lease are badged with a trailing `⚠ interrupted` marker (and an additive `interrupted: true` field on their `--json` row) so they are visible at a glance without running `tasks doctor`; the lease check is skipped entirely when no `.archloop/locks/` directory exists, so a healthy board incurs no extra cost. Done tasks are truncated by default; pass `--all` to expand them. Use `--warning high|medium|low` to filter PRD-warning tasks. Pass `--json` for a structured array of rows (including `remoteBadge` and `interrupted` when present). Task commands target the selected Hub project by default and accept `--project <name>` for an explicit override.
...
Stale-execution recovery is event-aware: it consults the run event log so a finished phase is never redone. A task whose review already completed (`task_review_succeeded`) resumes at `waiting_for_merge` with its claim preserved (the merge was interrupted); a task whose implementation completed (`task_implementation_succeeded`, reviewer flow) resumes at `reviewing` with its claim preserved; a task interrupted with no phase-completion event retries from `ready_for_agent`, dropping the claim and reusing the preserved worktree when the branch already has commits.

### `archloop tasks recover --stale`

Recover every interrupted-execution task on the board in one batch instead of one task at a time. With `--stale`, archLoop loads the task board, worktree leases, and run event log; flags tasks stuck in `implementing` / `reviewing` / `merging` with no live worktree lease; and routes each through the same event-aware recovery as the single-task form. By default it prints a dry-run preview of the planned routing per task (prior status -> target, claim preserved or released, and the reason) so you can verify nothing will be re-implemented unnecessarily; it applies the recovery only when you pass `--yes` (or confirm interactively in a TTY), mirroring `tasks repair-state`. Pass a `<task-selector>` (without `--stale`) for single-task recovery.

### `archloop tasks doctor`

Audits local Beads task-board state against Hub run events and git branch/worktree state without mutating Beads, git, or remote GitHub Issues. It reports multiple archLoop status labels, stale `metadata.hubStatus`, missing execution claim fields, failed tasks that still have branch work, merge-ready run history that is not selectable for merge, terminal tasks that still carry execution claim metadata, dirty source worktree gates, worktree lease claim/occupancy mismatches (active execution, stale lease with failed claim, active lease without claim), interrupted executions (`implementing` / `reviewing` / `merging` with no live worktree lease) that no lease diagnostic already covers — with a next action that distinguishes "review/merge already finished, recover to advance" from "implement was interrupted, recover to retry" — task state that still needs `archloop tasks push`, and managed branch cleanup diagnostics for safe candidates, blocked branches, and historical unowned preservation rules.

Doctor output groups diagnostics by severity and colors them when the terminal supports it: error (interrupted/failed work), warn (stale claims / pending sync), info (orphaned / informational cleanup). A badges row summarizes the count per severity, each group shows its symbol/color and count, and each item shows the task id, the reason, the recommended next action as a dim trailing hint, and the human message as a dim continuation line. Under `NO_COLOR`, non-TTY, or `--plain`, the same output flattens to grep-friendly plain text that still preserves every task id, reason, message, and next action. Each finding carries the next action for resolving it: rerun the flow, recover a failed or interrupted task (`archloop tasks recover <id>` / `recover --stale`), repair local state, push task sync, or clean up managed branches with `archloop tasks cleanup --yes` or `--include-unowned` when appropriate. Dirty source files are a Git safety warning, not repairable Beads task-state pollution. If a later `run --flow` needs to land merge-ready branch work that overlaps those dirty files, commit, stash, or discard the listed blocking files, then rerun the same flow so the batch resumes.
...
The `no-review` flow reads the Beads ready queue, selects a batch of eligible `ready_for_agent` tasks, claims the selected tasks, starts their implementers concurrently with task id/title/branch supplied by TypeScript orchestration, and advances successful work to `waiting_for_merge`. Implementer success is derived from observed branch commits (or existing unmerged work) plus a `<promise>COMPLETE</promise>` signal — a trailing non-zero provider exit does not override that, and failure events report the true branch `commitCount`. A completion signal with no new branch work is also treated as success when the task's implementation was already merged into the base branch (a prior `merge_succeeded` / `task_closed` run event), so a faithful zero-new-commit re-run of already-merged work is closed as `done` instead of `agent_failed`; the completion signal alone is never trusted, so a genuine no-work failure still fails. Mid-run provider transients (`API Error: 400 Invalid request parameters`, Cursor `RetriableError: Connection stalled` / `PING timed out`) are retried with exponential backoff (defaults: 3 attempts, 5s / 20s / 60s; override with `ARCHLOOP_PROVIDER_RETRY_ATTEMPTS` and `ARCHLOOP_PROVIDER_RETRY_BASE_MS`) and recorded as `task_provider_retry` events; if `<promise>COMPLETE</promise>` was already emitted, observation wins and no retry runs. After a successful batch completes, Hub reloads the ready queue and continues with the next batch until the queue is empty or `--max-batches` is reached. Agent or sandbox failures move tasks to `failed` with a failure reason.
...
At `archloop run --flow <id>` startup, Hub warns early when the source worktree has dirty source files. Before planning a fresh batch, Hub also scans for interrupted executions — tasks stuck in `implementing` / `reviewing` / `merging` with no live worktree lease — applies the event-aware recovery router to each one, and reports how many were auto-recovered and to which phase each was routed (full per-task summary in human output; a structured `autoRecover` object on the JSON `run_completed` record). Tasks routed to `waiting_for_merge` (claim preserved) are then picked up by the resumed-batch merge path; tasks routed to `ready_for_agent` are selected by the planner and re-implemented, reusing the preserved worktree when the branch already has commits. A recovery to `reviewing` preserves the finished implementation so it is never re-implemented. This startup auto-recovery is the deliberate ADR-0034 exception to explicit recovery; other failed or inconsistent task states still require `archloop tasks recover` / `repair-state` and are not silently mutated.

If the same flow has an unfinished previous batch with tasks still in `waiting_for_merge`, the run creates a fresh Hub run directory for the retry and resumes that old batch's merge selection by its original batch id before planning or claiming new `ready_for_agent` tasks. Ready tasks that exist at the same time stay unclaimed until the resumed merge batch completes. When the host worktree is dirty, Hub checks whether the pending merge-ready branches would touch the same paths and lists exact blocking files before any fresh tasks are claimed. If that resumed merge selection is blocked by stale state, inconsistent state, or dirty-file overlap, Hub stops the flow with `batch_failed` instead of falling through to a fresh queue scan.
