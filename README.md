...
Processes `inbox` and `needs_info` Hub tasks through the first triage workflow. It classifies each task into `needs_info`, `ready_for_agent`, `ready_for_human`, or `wontfix`, updates Beads labels/metadata to match, closes wontfix tasks locally, and appends concise AI triage comments that start with `> *This was generated by AI during triage.*`.
