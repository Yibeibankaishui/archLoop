...
The `no-review` flow reads the Beads ready queue, selects a batch of eligible `ready_for_agent` tasks, claims the selected tasks, runs an implementer with task id/title/branch supplied by TypeScript orchestration, and advances successful work to `waiting_for_merge`. Agent or sandbox failures move tasks to `failed` with a failure reason.
...
Task-board flows treat `ready_for_agent` as the **candidate pool**, not the automatic execution set. The **selected flow batch** is the subset chosen by the configured batch strategy before claim. By default, `no-review` and `with-review` use the `planned` strategy with a maximum of **3** tasks. Override with `--batch-strategy` and `--max-tasks`:

| Strategy       | Behavior |
| -------------- | -------- |
| `planned` (default) | Uses a flow-owned batch planner to choose a safe parallel subset when available. Planner failures or invalid output fall back to `conservative` selection and record a fallback reason in Hub batch events and CLI output. |
...
If a later `archloop run . --flow <id>` finds no selected `ready_for_agent` tasks but does find an unfinished previous batch for the same flow with tasks still in `waiting_for_merge`, it creates a fresh Hub run directory for the retry and resumes the old batch's merge selection by its original batch id (**resume-first**). If ready tasks exist at the same time as unfinished previous batches, archLoop starts a normal new batch and reports the old batch ids as not resumed instead of silently merging unrelated old work.
