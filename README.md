...
`hub-desktop/` ships a committed `package-lock.json`, so `npm ci` gives the
deterministic install path used by QA and CI. Run `npm run audit:critical` in
that directory to verify the desktop dependency tree stays clean.
...
The `no-review` flow reads the Beads ready queue, selects a batch of eligible `ready_for_agent` tasks, claims the selected tasks, starts their implementers concurrently with task id/title/branch supplied by TypeScript orchestration, and advances successful work to `waiting_for_merge`. Agent or sandbox failures move tasks to `failed` with a failure reason.
