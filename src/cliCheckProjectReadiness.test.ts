...
const runCli = async (args: string[]): Promise<ReadonlyArray<DisplayEntry>> =>
  Effect.runPromise(
    Effect.gen(function* () {
...
describe("archloop check project readiness", () => {
  ...
  it("checks an explicit project without the Hub slice", async () => {
    ...
    expect(
      mockCollectHubProjectReadinessCheck.mock.calls[0]?.[0],
    ).toMatchObject({
      name: "alpha",
    });
  });
  ...
  it("checks every registered project with --all-projects", async () => {
    ...
    expect(
      mockCollectHubProjectReadinessCheck.mock.calls.map(
        (call) => call[0].name,
      ),
    ).toEqual(["alpha", "beta"]);
  });
