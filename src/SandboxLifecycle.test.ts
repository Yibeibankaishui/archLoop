...
describe("withSandboxLifecycle (worktree mode)", () => {
  const setupWorktree = async () => {
    ...
    const worktreesDir = join(hostDir, ".archloop", "worktrees");
    await mkdir(worktreesDir, { recursive: true });
    const worktreeDir = join(worktreesDir, "test-worktree");
    await execAsync(
      `git worktree add -b "archloop/test" "${worktreeDir}" HEAD`,
      { cwd: hostDir },
    );
  ...
  it("temp branch is deleted after cherry-pick", async () => {
    ...
    const { stdout } = await execAsync('git branch --list "archloop/test"', {
      cwd: hostDir,
    });
  ...
  it("cherry-pick works when sandboxRepoDir differs from host worktree path", async () => {
    ...
    // sees /home/agent/workspace while the host sees .archloop/worktrees/<name>.
    //
      ...
      withSandboxLifecycle(
        ...
        (ctx) =>
          Effect.gen(function* () {
            ...
            yield* ctx.sandbox.exec(
              'sh -c "git checkout archloop/test && git merge --no-ff feature/merge-test -m \\"Merge feature/merge-test\\""',
              { cwd: ctx.sandboxRepoDir },
            );
          }),
      ).pipe(Effect.provide(Layer.merge(layer, testDisplayLayer))),
      ...
      withSandboxLifecycle(
        ...
        (ctx) =>
          Effect.gen(function* () {
            ...
            yield* ctx.sandbox.exec(
              'sh -c "git checkout archloop/test && git merge --no-ff branch-a -m \\"Merge branch-a\\""',
              { cwd: ctx.sandboxRepoDir },
            );
  ...
  it("no cherry-pick when explicit branch is given", async () => {
    ...
    expect(result.branch).toBe("archloop/test");
    expect(result.commits).toHaveLength(1);

    // Commit is on archloop/test, NOT cherry-picked to main
