 ...
 *
 * Usage:
 *   import { vercel } from "archloop/sandboxes/vercel";
 ...
 * plus archLoop-specific options for auth and branch strategy.
 */
...
export const vercel = (options?: VercelOptions): IsolatedSandboxProvider =>
  createIsolatedSandboxProvider({
    ...
    create: async (createOptions): Promise<IsolatedSandboxHandle> => {
      ...
      // Merge provider env with archLoop env
      ...
      const handle: IsolatedSandboxHandle = {
        ...
        copyIn: async (
          ...
          if (info.isDirectory()) {
            const tarPath = join(
              tmpdir(),
              `archloop-copyin-${Date.now()}.tar.gz`,
            );
            execSync(`tar -czf "${tarPath}" -C "${hostPath}" .`);
            try {
              const tarContent = await readFile(tarPath);
              const sandboxTarPath = `/tmp/archloop-copyin-${Date.now()}.tar.gz`;
