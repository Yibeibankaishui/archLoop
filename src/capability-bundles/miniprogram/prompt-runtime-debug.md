...
The **runtime-debug** add-on is enabled for this repository. Read `.sandcastle/context/miniprogram-runtime-debug.md` when — and only when — the task needs simulator, debugger, screenshot, console, or page-state evidence.

Rules:

- Do **not** start WeChat Developer Tools or MCP servers for every task.
- Prefer `.sandcastle/verify.sh` and the verification summary contract unless runtime debugging evidence is explicitly required.
- Runtime debugging does not replace `.sandcastle/verify.sh` and is not a completion standard by itself.
