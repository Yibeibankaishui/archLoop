...
describe("hubProjectReadinessCheck", () => {
  it("reports repo, git, contract, task-store, run, and flow readiness for a selected project", async () => {
        ...
        failedTasks: [
          {
            ...
            nextAction: "archloop tasks recover bd-2 to reset and retry agent work",
          },
        ],
        syncCounts: {
          pushPending: 0,
    ...
    const output = (
      await import("./hubProjectReadinessCheck.js")
    ).formatHubProjectReadinessCheckLines(project, report).join("\n");
  ...
  it("fails when the selected project repo path is missing", async () => {
    const repoDir = await mkdtemp(join(tmpdir(), "hub-project-readiness-missing-"));
    await initRepo(repoDir);
    await commitFile(repoDir, "hello.txt", "hello", "initial commit");
