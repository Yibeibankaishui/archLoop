...
任务看板 flow 默认使用 `--output auto`。能力足够的交互式 TTY 会显示有界 live view：紧凑 run header、稳定的当前 batch 任务行、规范阶段、elapsed、durable logs、已完成 batch scrollback，以及包含五类任务计数的最终 outcome。它不进入 alternate screen，也不显示原始 agent prose、tool 参数、百分比或 ETA；终端 resize 时会在宽布局与堆叠布局间切换，并在成功、失败、取消或异常清理时恢复光标。stdout 重定向、CI、`TERM=dumb`、不支持 cursor control 或终端宽度、高度不安全时自动回退为 plain。在 live view 中，`NO_COLOR` 或 `--no-color` 只关闭颜色，不删除文本标签和状态符号。

任务看板 flow 也可显式使用稳定的纯文本输出：`archloop run --flow no-review --output plain` 或 `archloop run --flow with-review --output plain`。该模式让每条 lifecycle 或 task-attention 记录各占一个物理行，字段顺序稳定、值会转义，不使用 ANSI 光标重写；输出会包含 Hub project、flow、所选任务、用户可读阶段、run id、Hub run directory，以及 completed、failed、blocked、skipped、ready-to-merge 五类计数。最终结果区分 `completed`、`completed_with_failures`、`failed` 和 `cancelled`；初始队列为空会显示 `Nothing to run`，达到 `--max-batches` 也视为成功，二者退出码为 `0`，阻塞或部分失败为非零，用户取消为 `130`。失败详情保留阶段、简短诊断和日志路径：agent、sandbox、implementation 或 review 失败提示 `archloop tasks recover <selector>`，状态不一致提示 `archloop tasks repair-state <selector>`，dirty overlap 列出准确路径并提示 commit、stash 或 discard。`waiting_for_merge` 工作只需重新运行同一个 flow，archLoop 不提供新的 `run --resume` 契约。agent prose、tool 参数和直接的 agent 启动装饰不会混入纯文本生命周期输出，详细内容仍保存在 Hub run directory。当前 `plain` 仅支持 `no-review` 与 `with-review`。

自动化消费可使用 `archloop run --flow <no-review|with-review> --output json`。该模式只向 stdout 写 JSONL，每个物理行都是一个 schema version 1 JSON 对象；记录包含稳定的输出 identity/sequence、时间戳、run/flow/batch/task identity，以及 lifecycle data。失败任务通过 `task_attention` 提供阶段、简短诊断、日志、恢复命令和 blocking paths；最终 `run_completed` 提供 outcome、五类任务计数、完成的 batch/task 数、停止原因、日志目录和退出码。取消使用 `cancelled` outcome 与退出码 `130`。prompt、人工装饰、ANSI 控制序列、agent 启动文本和原始 agent 输出不会污染 stdout；JSON 字符串转义可安全承载空白、换行、引号、路径和诊断。schema version 1 允许增加字段，消费者应忽略未知字段。
