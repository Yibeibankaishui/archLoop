...
1. **Link** (first step — do not `npm install` archLoop):
   ...
   `npx archloop project configure --project-profile node` creates or updates the Hub project development contract under the Hub project assets directory. Omit `--project-profile` in a TTY to pick from a prompt; non-interactive runs must pass the flag explicitly.
