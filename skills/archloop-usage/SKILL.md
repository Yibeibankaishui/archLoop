---
name: archloop-usage
description: Set up and run archLoop (@yibeibankaishui/archloop) to orchestrate AI coding agents in isolated sandboxes inside another Git project. Use when a user wants to add, configure, initialize, init, build, or run archLoop in a target repo; mentions @yibeibankaishui/archloop, `archloop initialize`, `archloop init`, `.archloop/`, parallel-planner, AFK agents, or sandbox-based agent orchestration; or hits errors from a archLoop run (gh 401, missing bootstrap.sh, exit 127, dirty merge overlap diagnostics, merger stash issues).
---
...
1. **Link** (first step — do not `npm install` archLoop):
   ...
   If you want to inspect or register Hub projects before project-local init, `npx archloop initialize` configures shared Hub agent roles, env values, and auth guidance from any directory, runs the quick Hub check by default, and ends with `npx archloop project add` as the next step (`--skip-check` skips the smoke call). `npx archloop project add --name <name> --path <repo>` registers an existing repo from any directory, `npx archloop project list` shows the shared registry, selected Hub project, repo path, profile, path validity, task readiness labels, ready/failed/total counts when available, and active run presence, `npx archloop project select <name>` updates the CLI default target, `npx archloop project rename <project> <new-name>` renames a Hub project without changing its stable id, `npx archloop project relink <project> --path <repo-path>` updates the repo path after validating the new repo has an initial commit, `npx archloop project status [--project <name>]` resolves the explicit or selected Hub project target first and then shows the canonical repo root, archLoop user data dir, Hub project directory, selected Hub project profile, contract path, `bd` availability, task-store initialization status, task counts by Hub status, active runs and batch status, failed tasks with next actions, sync state summaries, recent Hub events, worktree lease diagnostics for Hub-owned executions, managed branch cleanup diagnostics, and paths to Hub run directories for full logs. `npx archloop check` defaults to Hub-wide checks plus the selected Hub project when one exists; use `--hub`, `--project <name>`, or `--all-projects` to narrow or broaden the target. It shows visible progress for each validation step, performs grouped provider/model smoke checks through the real provider path, and project checks validate repo path existence, git repo validity, initial commit presence, development contract state, local task store state, ready/failed task summary, active run presence, and flow readiness signals.
