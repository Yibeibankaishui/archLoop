...
1. **Link** (first step — do not `npm install` archLoop):
   ...
   - `npx archloop initialize` configures shared Hub agent roles, env values, and auth guidance from any directory, runs the quick Hub check by default, and ends with `npx archloop project add` as the next step. Pass `--skip-check` to skip the smoke call.
...
2. **Hub onboarding first**: `npx archloop initialize`. Interactive prompts: Hub agent roles, shared env values, auth guidance, then a quick Hub check and the next step `archloop project add`.
...
| `archloop initialize [--skip-check]`                                                                                                                                 | Configure shared Hub agent roles, env values, and auth guidance; run the quick Hub check by default; end with `archloop project add`.                                                       |
