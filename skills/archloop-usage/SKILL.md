...
1. **Link** (first step — do not `npm install` archLoop):
   ...
   If you want to inspect or register Hub projects before init, `npx archloop project add --name <name> --path <repo>` registers an existing repo from any directory, `npx archloop project list` shows the shared registry and selected Hub project, `npx archloop project select <name>` updates the CLI default target, `npx archloop project status` shows the canonical repo root, archLoop user data dir, Hub project directory, selected Hub project profile, contract path, `bd` availability, task-store initialization status, task counts by Hub status, active runs and batch status, failed tasks with next actions, sync state summaries, recent Hub events, worktree lease diagnostics for Hub-owned executions, and paths to Hub run directories for full logs. `npx archloop check --hub` runs the Hub-wide readiness slice from any directory, shows visible progress for each static validation step, and currently warns that provider/model smoke checks are deferred until the next slice.
...
| `archloop project add --name <name> --path <repo>`                      | Register an existing git repo as a Hub project                          |
| `archloop project list`                                                 | Show registered Hub projects and the selected project                   |
...
- **`archloop check --hub` missing roles or credentials**: The Hub readiness slice validates role completeness, shared credentials/auth, provider references, and provider CLI availability without making provider/model smoke calls yet. It prints exact follow-up commands for missing roles or credentials, and warns that smoke checks are deferred until a later slice. Use `archloop agent-config show`, `archloop env show`, and `archloop auth show` to inspect current Hub state.
