...
1. **Link** (first step — do not `npm install` archLoop):
   ...
   If you want to inspect or register Hub projects before init, `npx archloop project add --name <name> --path <repo>` registers an existing repo from any directory, `npx archloop project list` shows the shared registry and selected Hub project, `npx archloop project select <name>` updates the CLI default target, and `npx archloop project status` shows the canonical repo root, archLoop user data dir, Hub project directory, selected Hub project profile, contract path, `bd` availability, task-store initialization status, task counts by Hub status, active runs and batch status, failed tasks with next actions, sync state summaries, recent Hub events, worktree lease diagnostics for Hub-owned executions, and paths to Hub run directories for full logs.
