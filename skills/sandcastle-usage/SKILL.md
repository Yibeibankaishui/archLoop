...
- **Hub flow merge failure / `failed(merge_failed)`**: `sandcastle run . --flow ...` now includes the first useful Git diagnostic in the merge summary and Hub events. Use it to decide whether to clean the host worktree, inspect the branch state, or rerun `sandcastle tasks recover <selector>` before retrying the flow.
