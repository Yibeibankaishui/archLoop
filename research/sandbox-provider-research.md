...
Two provider types in archLoop's architecture (see [UBIQUITOUS_LANGUAGE.md](./UBIQUITOUS_LANGUAGE.md)):
...
3. [Not Viable](#not-viable-for-archloop)
...
The incumbent. Universal image ecosystem. Requires a daemon. Already supported by archLoop.
...
Powers AWS Lambda and Fargate. Fast boot (<125ms). But **no filesystem sharing** — only block devices. A Firecracker-based archLoop provider would have to be an **isolated provider** (git sync). A WIP virtiofs PR was never merged.
...
Fastest Docker Desktop alternative on macOS. Provides a Docker socket — works transparently with archLoop's Docker provider.
...
Disposable Windows desktop. The lack of process I/O capture makes it **unusable for archLoop's `exec` model**.
...
3. **Daytona** — Best isolated provider candidate. 90ms startup, TS SDK, built-in Git support (simplifies sync), AGPL (self-hostable). The Git integration means archLoop's bundle/patch sync could potentially be replaced by Daytona's native Git ops.
